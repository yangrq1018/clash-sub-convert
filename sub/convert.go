...
var (
	...
	RulesEmby = []Rule{
		DomainSuffixRule("mb3admin.com", embyGroup),
		DomainKeyWordRule("emby", embyGroup), // emby server of TAG
	}
...
// 对github内容的访问本身需要代理，所以在运行代理生成程序的时候，clash需要已经监听7890端口
...
func semporiaClashXRules(key string) RuleProvider {
	return RuleProvider{
		...
		URL:      "https://cdn.jsdelivr.net/gh/Semporia/Clash-X@master/Filter/" + key + ".yaml",
		Interval: 3600,
	}
}
...
// 按国家分组
func groupByCountries(remote ClashSub, gr *ProxyGroup) (countryGroups []ProxyGroup) {
...
func Rewrite(remote ClashSub, out io.Writer, proc ...Processor) error {
	...
	gr := grand()
	an := allNodes(remote)
	countryGroups := groupByCountries(remote, &gr)
